...
The ingredient keys below - `recipe`, `input`, `template`/`base`/`addition`, and the rest - are the same ones every eco plugin uses, and are documented in full on the [Recipes](https://hub.auxilor.io/wiki/eco/the-item-lookup-system-recipes) page.
...
- **Recipe formats:** [Recipes](https://hub.auxilor.io/wiki/eco/the-item-lookup-system-recipes) for the shared reference on ingredient keys and which plugins support what.
